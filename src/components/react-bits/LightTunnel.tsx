...
/*
 * Vendored from React Bits (https://reactbits.dev/r/LightTunnel-TS-TW), MIT +
 * Commons Clause. Pulled from the registry by hand rather than by the shadcn
 * CLI — the CLI rewrites `motion` down to ^12 on every run in this repo, and
 * it would have dropped the file at `components/LightTunnel/LightTunnel.tsx`
 * rather than beside the other vendored effects. The payload is otherwise the
 * one `shadcn add https://reactbits.dev/r/LightTunnel-TS-TW` would have
 * fetched, and its single dependency, `ogl@^1.0.11`, is installed.
 *
 * Changes from upstream, marked `EDIT:`:
 *
 *   1. `"use client"` — upstream targets Vite and ships no directive. Without
 *      it the Next build fails on the hooks below.
 *   2. The uniform bag was typed `Record<string, { value: any }>`, which trips
 *      `@typescript-eslint/no-explicit-any`. Every uniform this component
 *      writes is a number or a `Float32Array`, so it is typed as that.
 *   3. `currentMouse` was `let` but is only ever written through its indices,
 *      which `prefer-const` fails the build on.
 *   4. Two `cond ? start() : stop()` statements became `if`/`else`. They lint
 *      as unused expressions; the behaviour is unchanged.
 *
 * A note on what this file does NOT do. It runs an unconditional
 * `requestAnimationFrame` loop, and it reads the palette as hex strings that
 * it converts to shader uniforms — it cannot see the `oklch()` brand tokens.
 * Both are the caller's problem to solve, and `landing-hero.tsx` solves them:
 * it does not mount this component at all under `prefers-reduced-motion` or on
 * a coarse pointer, and it passes sRGB hex literals that are pinned to the
 * brand ramp in a comment there.
 *
 * It does throttle itself in the two cases it can see on its own: an
 * `IntersectionObserver` stops the loop when the canvas scrolls off, and a
 * `visibilitychange` listener stops it when the tab is hidden.
 */
...
const LightTunnel: React.FC<LightTunnelProps> = ({
  ...
  useEffect(() => {
    ...
    // EDIT: `let` upstream, but only ever mutated through its indices.
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        ...
        // EDIT: a bare `cond ? a() : b()` upstream, which lints as an unused
        // expression.
    ...
    const onVisibility = () => {
      ...
      // EDIT: same unused-expression ternary as above.
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const { program } = ctx;
    // EDIT: `{ value: any }` upstream; every uniform written below is one
    // of these two shapes.
