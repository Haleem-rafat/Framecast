...
/*
 * Vendored from React Bits (https://reactbits.dev/r/GradientWaves-TS-TW),
 ...
 *   2. `prefers-reduced-motion` — the caller decides whether to mount it at
 *      all (see `v3-hero.tsx`), because this is a raymarched WebGL loop with
 *      no internal pause.
 *
 * Its three colours are hex strings bound to shader uniforms, so it cannot
 * read the `oklch()` brand tokens directly and the caller converts. That is
 * the standing cost of a WebGL background in a design system built on CSS
 * variables — the same one Threads pays on /v2.
 */
...
const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        ...
        // EDIT: an `if`, not a ternary evaluated for its side effects, which
        // `next/typescript` flags as an expression statement.
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    const { program } = ctx;
    // EDIT: `unknown`, not `any`. Every write below assigns a concrete number
    // or array to `.value`, so nothing needs the escape hatch and
    // `next/typescript` bans it.
