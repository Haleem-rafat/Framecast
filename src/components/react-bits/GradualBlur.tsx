...
/*
 * Vendored from React Bits (https://reactbits.dev/r/GradualBlur-TS-TW), MIT +
 * Commons Clause, no third-party dependency — it is a stack of
 * `backdrop-filter` layers under a mask. Copied in by the shadcn CLI. The only
 * change from upstream is `"use client"`.
 *
 * Used purely as an edge treatment: it softens the bottom of the horizontal
 * stage track so the cards fade out rather than being cut off by the viewport.
 * It is `pointer-events: none` and `aria-hidden`, and nothing is ever read
 * through it.
 */
