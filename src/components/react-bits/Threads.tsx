...
/*
 * Vendored from React Bits (https://reactbits.dev/r/Threads-TS-TW), MIT +
 ...
 *      all (see `v2-hero.tsx`), because a WebGL loop with no pause is not
 *      something to leave running for someone who asked for less movement.
 *
 * Kept from upstream, and better than most of the library's backgrounds: it
 * already gates its own frame loop on an IntersectionObserver and
 * `document.hidden`, so it stops rendering once it scrolls off screen.
 *
 * `color` is `[r, g, b]` in 0–1, not a CSS colour, so it cannot read the
 * `oklch()` brand tokens directly — the caller converts. That is the standing
 * cost of a WebGL background inside a design system built on CSS variables.
 */
