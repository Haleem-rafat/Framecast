...
/*
 * Vendored from React Bits (https://reactbits.dev/r/ScrollExpand-TS-TW),
 ...
 *   2. `mediaSlot` — upstream always renders `<img src={src}>` (or a
 *      `<video>`), and with no `src` that is a broken image element on the
 *      page. This product has no screenshots it is allowed to show: every
 *      frame on these pages is drawn in CSS and labelled as an illustration,
 *      because a mocked-up still of a video nobody made would be a lie told in
 *      pictures. `mediaSlot` puts that drawn frame in the media layer instead.
 *   3. `prefers-reduced-motion` — the clip-path stays at its final, fully
 *      expanded state and no scroll listener is attached, so the frame is
 *      simply there rather than growing as you scroll past it.
 */
...
export interface ScrollExpandProps {
  /** EDIT: rendered in place of the `<img>`/`<video>`. See note 2 above. */
  mediaSlot?: ReactNode;
  src?: string;
...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  useEffect(() => {
    ...
    // EDIT: see note 3 at the top of the file. The frame is pinned at its
    // final, fully expanded state and no listener is attached, so it does not
    // grow as the page scrolls past it — it is simply already open.
  ...
  const media = mediaSlot ? (
    <div className="absolute inset-0 h-full w-full origin-center select-none [will-change:transform]">
      {mediaSlot}
    </div>
  ) : mediaType === 'video' ? (
      <video
      ...
      // EDIT: `next/image` cannot be used here — the component hands this
      // element to GSAP-free imperative transform code via `mediaRef` and
      // needs a raw `HTMLImageElement`. This branch is unused on /v3, which
      // passes `mediaSlot` instead.
