...
/*
 * Vendored from React Bits (https://reactbits.dev/r/WarpText-TS-TW), MIT +
 * Commons Clause. Pulled from the registry by hand for the same reasons as
 * `LightTunnel.tsx` — the shadcn CLI rewrites `motion` to ^12 in this repo.
 * The Tailwind variant is taken rather than the CSS one so there is no
 * stylesheet to import beside it; `ogl@^1.0.11` is already a dependency.
 *
 * Changes from upstream, marked `EDIT:`:
 *
 *   1. `"use client"` — upstream targets Vite and ships no directive.
 *   2. The root element was `role="img"` with an `aria-label` of the text.
 *      That is the right default for a component dropped in on its own, and
 *      the wrong one here: this renders the *same words* as a real `<h1>` that
 *      stays in the DOM behind it, so announcing them again would read the
 *      headline twice. The root is `aria-hidden` instead and the caller owns
 *      the accessible name. See `landing-hero.tsx`.
 *   3. `min-h-[220px]` was dropped from the root. Upstream needs a floor
 *      because the component is usually the only thing in its box; here it is
 *      laid over an `<h1>` whose own height defines the space, and a 220px
 *      floor would have pushed the hero's buttons down the page.
 *
 * This component rasterises text into a 2D canvas and feeds it to a shader.
 * Two consequences the caller has to design around, and `landing-hero.tsx`
 * does: what it draws cannot be selected or read by a crawler, and it does not
 * wrap — `text` is split on `\n` and nothing else, so line breaks are the
 * caller's decision at every width it is allowed to run at.
 *
 * It reads `prefers-reduced-motion` itself and freezes its own clock. That is
 * not enough on its own — a frozen canvas is still a WebGL context and still a
 * download — so the caller additionally refuses to mount it at all. See
 * `useAmbientEffectsAllowed`.
 */
...
const WarpText = ({
  ...
  useEffect(() => {
    ...
    // EDIT: upstream declared `geometry`, `mesh` and `texture` as uninitialised
    // `let`s here and assigned each exactly once below, which `prefer-const`
    // fails the build on. They are now `const` at their assignment.
  ...
  return (
    <div
      ref={containerRef}
      // EDIT: upstream also sets `min-h-[220px]`; see the note at the top.
      className={`relative block w-full overflow-hidden isolate ${className}`.trim()}
      style={style}
      // EDIT: `role="img"` + `aria-label={text}` upstream. The caller renders
      // the same words as real text; this layer is decoration over them.
      aria-hidden="true"
    />
  );
};
